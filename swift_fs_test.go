...
func TestFileread(t *testing.T) {
	...
	_, err := generateTestObject(filename, 1024*1024)
	if err != nil {
		t.Error(err)
	}
...
func TestFilecmd(t *testing.T) {
	...
	r, _, err := s.Download(targetName)
	content, _ := ioutil.ReadAll(r)
	if !reflect.DeepEqual(content, data) {
		t.Errorf("Wrong downloaded data")
	}

	if err = s.Delete(targetName); err != nil {
		t.Error(err)
	}

	if _, err = s.Get(targetName); err == nil {
